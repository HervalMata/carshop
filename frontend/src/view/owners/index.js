...
import { AppBar, Fab, IconButton, Toolbar, Typography, Slide, Fade, CircularProgress, List, ListItem, ListItemAvatar, Avatar, ListItemText, Menu, MenuItem, Divider } from '@material-ui/core'
                ...
                {(isLoading) ? <div className="d-flex justify-content-center mt-5 pt-5"> <CircularProgress/> </div> :
                    ...
                    {(owners.data.length > 0) && 
                        <div className="card-body">
                            <h6 className="m-0">{owners.total} {(owners.total > 1) ? 'proprietários encontrados' : 'proprietário encontrado'}</h6>
                        </div>
                    }
                    ...
                    {(owners.data.length < 1) && 
                        <div className="text-center mt-5 mb-5 pt-5 pb-5">
                            ...
                            <h6 className="mt-4 text-muted">Nenhum proprietário encontrado</h6>
                        </div>
                    }
                            ...
                            <React.Fragment key={index}>
                                <ListItem button selected={state.isDeleted === item.id}>
                                    <ListItemAvatar onClick={() => _show(item)}>
                                        <Avatar className="bg-primary">{item.name.slice(0,1)}</Avatar>
                                    </ListItemAvatar>
